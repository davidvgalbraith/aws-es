...
AWSES.prototype.mget = function(options, callback) {
    ...
    if( !is.existy(options.ids) ) return callback('not_ids');
    if( !is.array(options.ids) ) return callback('invalid_ids');

    var path = '/' + options.index + '/' + options.type + '/_mget';
    self._request(path, {ids: options.ids}, function(err, data) {
        if(err) return callback(err);
        return callback(null, data);
    });
};
